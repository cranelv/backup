...
func (p *Process) sendFullBlockReq(hash common.Hash, number uint64, target common.Address) {
	if p.FullBlockReqCache.IsExistMsg(hash) {
		...
		p.pm.hd.SendNodeMsg(mc.HD_V2_FullBlockReq, reqMsg,common.Address{}, common.RoleNil, []common.Address{target})
	} else {
...
func (p *Process) ProcessFullBlockReq(req *mc.HD_V2_FullBlockReqMsg) {
	...
	p.pm.hd.SendNodeMsg(mc.HD_V2_FullBlockRsp, rspMsg,common.Address{}, common.RoleNil, []common.Address{req.From})
}
