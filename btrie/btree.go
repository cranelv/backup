...
func NewFreeList(size int) *FreeList {
	return &FreeList{freelist: make([]*bnode, 0, size)}
}

func (f *FreeList) newNode() (n *bnode) {
	f.mu.Lock()
...
func (f *FreeList) freeNode(n *bnode) (out bool) {
	f.mu.Lock()
...
// insertAt inserts a value into the given index, pushing all subsequent values
// forward.
func (s *children) insertAt(index int, n *bnode) {
	*s = append(*s, nil)
	if index < len(*s) {
		copy((*s)[index+1:], (*s)[index:])
	}
...
// removeAt removes a value at a given index, pulling all subsequent values
// back.
func (s *children) removeAt(index int) *bnode {
	n := (*s)[index]
	copy((*s)[index:], (*s)[index+1:])
	(*s)[len(*s)-1] = nil
	*s = (*s)[:len(*s)-1]
...
func (n *bnode) mutableFor(cow *copyOnWriteContext) *bnode {
	if n.cow == cow {
		return n
	}
	out := cow.newNode()
...
func (n *bnode) mutableChild(i int) *bnode {
...
func (n *bnode) split(i int) (Item, *bnode) {
	item := n.items[i]
	next := n.cow.newNode()
...
func (n *bnode) maybeSplitChild(i, maxItems int) bool {
...
func (n *bnode) insert(item Item, maxItems int) Item {
	...
	//log.Info("btree", "insert", "begin==========================", "found", found)
	for Khash, _ := range item.(SpcialTxData).Value_Tx {
		log.Trace("btree", "insert:hash", Khash)
	}
	if found {
		n.items[i].InsertTxData(item)
		log.Trace("btree", "insert:found", found)
		//n.items[i] = item
		return n.items[i]
	}
	if len(n.children) == 0 {
		n.items.insertAt(i, item)
		log.Trace("btree", "insert:len(n.children)", len(n.children))
		return nil
	}
	if n.maybeSplitChild(i, maxItems) {
		...
		case item.Less(inTree):
			log.Trace("btree", "insert:item.Less(inTree)", "true")
			// no change, we want first split node
		case inTree.Less(item):
			log.Trace("btree", "insert:inTree.Less(item)", "true")
			i++ // we want second split node
		default:
			log.Trace("btree", "insert:default", "true")
			out := n.items[i]
			n.items[i] = item
			return out
		}
...
func (n *bnode) get(key Item) Item {
...
func (n *bnode) remove(item Item, minItems int, typ toRemove) Item {
	var i int
	var found bool
	switch typ {
	case removeMax:
...
// that we hit case A.
func (n *bnode) growChildAndRemove(i int, item Item, minItems int, typ toRemove) Item {
...
func (n *bnode) iterate(dir direction, start, stop Item, includeStart bool, hit bool, iter ItemIterator) (bool, bool) {
	var ok, found bool
	var index int
	switch dir {
	case ascend:
...
func (n *bnode) print(w io.Writer, level int) {
...
func BtreeSaveHash(node *bnode, db *trie.Database, typ byte, stateDB vm.StateDB) common.Hash {
	...
	for _, it := range node.items {
		...
		case common.ExtraTimeTxType:
			...
			if !ok {
				log.Error("btree", "BtreeSaveHash", "Assert SpcialTxData fail.Serious error.Serious error.Serious error.")
				return common.Hash{}
			}
...
func RestoreBtree(btree *BTree, itemNode *bnode, nodeHash common.Hash, db *trie.Database, typ byte, stateDB vm.StateDB) error {
	...
	if err != nil {
		log.Info("btree", "RestoreBtree:err", err)
...
func (c *copyOnWriteContext) newNode() (n *bnode) {
...
// documentation).
func (c *copyOnWriteContext) freeNode(n *bnode) freeType {
	if n.cow == c {
		// clear to allow GC
		n.items.truncate(0)
...
func (t *BTree) ReplaceOrInsert(item Item) Item {
	...
	if t.root == nil {
		log.Info("btree", "ReplaceOrInsert:t.root==nil", "if")
		...
		log.Info("btree", "ReplaceOrInsert:t.root!=nil", "else")
		if len(t.root.items) >= t.maxItems() {
			log.Info("btree", "ReplaceOrInsert:t.root!=nil", "else if")
	...
	log.Info("btree", "ReplaceOrInsert:t.root!=nil", "")
...
func (n *bnode) reset(c *copyOnWriteContext) bool {
...
func (a SpcialTxData) InsertTxData(b Item) bool {
	for txhash, val := range b.(SpcialTxData).Value_Tx {
		log.Trace("btree", "InsertTxData:len()", len(a.Value_Tx), "hash", txhash)
		a.Value_Tx[txhash] = val
		log.Trace("btree", "InsertTxData:len()", len(a.Value_Tx), "a.key", a.Key_Time)
	}
	return true
}
