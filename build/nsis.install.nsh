Name "gman ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install gman binary
Section "gman" gman_IDX
  SetOutPath $INSTDIR
  file {{.gman}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\gman.exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\gman.exe" "attach" "" ""
...
Function GetInstalledSize
  ...
  ${if} ${SectionIsSelected} ${gman_IDX}
    SectionGetSize ${gman_IDX} $0
