...
func GenerateChain(config *params.ChainConfig, parent *types.Block, engine consensus.Engine, db mandb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	...
	genblock := func(i int, parent *types.Block, statedb *state.ShardingStateDB) (*types.Block, types.Receipts) {
	...
	for i := 0; i < n; i++ {
		statedb, err := state.NewSharding(parent.Root(), state.NewDatabase(db))
		if err != nil {
			panic(err)
		}
