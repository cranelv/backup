...
func (api *PublicMinerAPI) SubmitWork(nonce, solution, digest, minerAddr, dataType string, workid string) bool {
	return api.agent.SubmitWork(nonce, solution, digest, minerAddr, dataType, workid)
}
...
func (api *PublicMinerAPI) GetWork() ([6]string, error) {
	if !api.e.IsMining() {
		return [6]string{}, errors.New("not mining work now")
	}
