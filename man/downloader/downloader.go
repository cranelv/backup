...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [man/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [man/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [man/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [man/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [man/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [man/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [man/63] Channel receiving inbound node state data
...
func (d *Downloader) Synchronise(id string, head common.Hash, td *big.Int, sbs uint64, sbHash common.Hash, mode SyncMode) error {
	err := d.synchronise(id, head, td, sbs, sbHash, mode)
	switch err {
	case nil:
	case errBusy:
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, sbs uint64, sbHash common.Hash, mode SyncMode) error {
	...
	return d.syncWithPeer(p, hash, td, sbs, sbHash)
}

// syncWithPeer starts a block synchronization based on the hash chain from the
// specified peer and head hash.
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int, pSbs uint64, sbHash common.Hash) (err error) {
	...
	superBLock, err := d.fetchHeight(p, sbHash)
	if err != nil {
		return err
	}
	...
	if 0 != pSbs {
		...
		if d.blockchain.Genesis().Hash() != sbHash {
			log.Error("创世文件不一致")
			return err
		}
	}
...
func (d *Downloader) fetchHeight(p *peerConnection, head common.Hash) (*types.Header, error) {
