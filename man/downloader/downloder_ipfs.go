...
func (d *Downloader) AddNewBatchBlockToIpfs() {
	...
	//var batchBodySize, batchZipSize int64
	...
	log.Warn(" ipfs AddNewBatchBlockToIpfs body info", "blockNum", d.dpIpfs.BatchStBlock.curBlockNum, "batchsize", batchBodySize, "zipsize", batchZipSize, "batchtoatalsize", gIpfsStat.totalBatchBlockSize, "zipTotalsize", gIpfsStat.totalZipBatchBlockSize)
...
func (d *Downloader) ParseBatchBody(batchblockhash string, beginReqNumber uint64, realBeginNum uint64, flg int) bool {
	...
	for {
		...
		//body改为stblock   obj := new(types.Body) bbody, err := rlp.EncodeToBytes(stBlock.Sblock.Body())
		obj := new(types.Block)
		//errd := rlp.Decode(blockFile, obj)
		errd := rlp.DecodeBytes(blockBuf, obj)
		if errd != nil {
