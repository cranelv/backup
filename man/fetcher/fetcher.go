	...
	for {
		...
		for hash, announce := range f.fetching {
			if time.Since(announce.time) > fetchTimeout {
				log.Trace("fetch f.fetching timeout", " blockNum", announce.number, "hash", hash, "origin peer", announce.origin)
				f.forgetHash(hash)
			}
		}
		//lb
		for hash, announce := range f.completing {
			if time.Since(announce.time) > 4*fetchTimeout {
				log.Trace("fetch f.completing timeout", " blockNum", announce.number, "hash", hash, "origin peer", announce.origin)
				f.forgetHash(hash)
				f.forgetBlock(hash)
			}
		}
		...
		case notification := <-f.notify:
			...
			if len(f.announced[notification.hash]) > 6 {
				log.Debug("fetch f.announced[] too big,discard ", "hash", notification.hash, "peer", notification.origin, "Number", notification.number)
				break
			}
			...
			for _, header := range task.headers {
				...
				if announce := f.fetching[hash]; announce != nil && announce.origin == task.peer && f.fetched[hash] == nil && f.completing[hash] == nil && f.queued[hash] == nil {
					...
					if header.Number.Uint64() != announce.number {
						log.Trace("Invalid block number header fetched", "peer", announce.origin, "hash", header.Hash(), "announced", announce.number, "provided", header.Number)
			...
			for i := 0; i < len(task.transactions) && i < len(task.uncles); i++ {
				...
				for hash, announce := range f.completing {
					if f.queued[hash] == nil {
						...
						//txnHash := types.DeriveSha(types.SelfTransactions(task.transactions[i]))
						//uncleHash := types.CalcUncleHash(task.uncles[i])
						log.Trace("download fetch bodyFilter map", "hash", hash, "number", announce.number, "announce tx", announce.header.TxHash, "txnHash", txnHash, "origin id", announce.origin)
						...
						log.Trace("fetch queued has this hash", "hash", hash)
					}
					//lb
					if dist := int64(announce.number) - int64(height); dist < -maxUncleDist || dist > maxQueueDist {
						log.Trace("fetch f.completing will delete hash", "hash", hash, "number", announce.number)
						f.forgetBlock(hash)
					}
				}
				if matched {
