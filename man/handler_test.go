...
// Tests that post man protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", man63, pm, true)
	defer peer.close()
	...
	if !timeout {
		blocks, _ := core.GenerateChain(&params.ChainConfig{}, genesis, manash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
			if remoteForked {
