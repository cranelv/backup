...
func newPeer(version int, p *p2p.Peer, rw p2p.MsgReadWriter) *peer {
	return &peer{
		...
		rw:          rw,
		version:     version,
		id:          fmt.Sprintf("%x%x", p.ID().Bytes()[:8],p.Self().Bytes()[:8]),
		knownTxs:    set.New(),
		knownBlocks: set.New(),
...
func (p *peer) readNewStatus(network uint64, status *statusNewData, genesis common.Hash) (err error) {
	msg, err := p.rw.ReadMsg()
	if err != nil {
//		log.Info("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA","errors",err)
		return err
	}
//	fmt.Println("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",msg)
	if msg.Code != StatusMsg {
		return errResp(ErrNoStatusMsg, "first msg has code %x (!= %x)", msg.Code, StatusMsg)
	}
	if msg.Size > ProtocolMaxMsgSize {
