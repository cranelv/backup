...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the manstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to manstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to manstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.man != nil {
		...
		if block == nil {
			block = s.man.BlockChain().CurrentBlock()
		}
		header = block.Header()
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to manstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.man != nil {
		mining = s.man.Miner().Mining()
		hashrate = int(s.man.Miner().HashRate())

		sync := s.man.Downloader().Progress()
		syncing = s.man.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock
