...
func (a *RemoteAgent) GetWork() ([6]string, error) {
	...
	if a.currentWork != nil {
		...
		res[3] = "pow"//a.currentWork.mineType.String()
		res[4] = block.Coinbase.Hex()
		res[5] = common.BytesToHash(n.Bytes()).Hex() //sm3
...
func (a *RemoteAgent) SubmitWork(strNonce, strAIHah, strHash, strMinerAddr, dataType string, strWorkid string) bool {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	case SubmitAiHash:
		...
		a.returnCh <- tmpHead //&Result{work, block.Header()}
		return true
	...
	//engine, exist := a.engine[string(result.Version)] //YYYYYYYYY
	//if exist == false {
