...
type SetRewardsExec interface {
	...
	SetMinerOutRewards(reward *big.Int, state util.StateDB, chain util.ChainReader, num uint64, innerMiners []mc.NodeInfo) map[common.Address]*big.Int
...
func (str *DefaultSetRewards) SetMinerOutRewards(reward *big.Int, state util.StateDB, chain util.ChainReader, num uint64, innerMiners []mc.NodeInfo) map[common.Address]*big.Int {

	return str.miner.SetMinerOutRewards(reward, state, num, chain, innerMiners)
}
